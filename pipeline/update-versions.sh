...
imageVersion () {
  if [[ "$TRIGGER" == *"navikt/"* && ( "$1" == *"$TRIGGER" || "$1" == *"$TRIGGER"/* ) ]]; then
    echo "$1:${TRIGGER_VERSION}"
  elif [[ "$1" == *"$TRIGGER" || "$1" == *"$TRIGGER"/* ]]; then
...
echo FPLOS_IMAGE="$(imageVersion "europe-north1-docker.pkg.dev/nais-management-233d/teamforeldrepenger/navikt/fp-los")" >> .env
