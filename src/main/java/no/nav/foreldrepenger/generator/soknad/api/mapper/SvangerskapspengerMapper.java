...
import static no.nav.foreldrepenger.generator.soknad.api.mapper.CommonMapper.tilOpptjening;
import static no.nav.foreldrepenger.generator.soknad.api.mapper.CommonMapper.tilVedlegg;
import static no.nav.foreldrepenger.generator.soknad.api.mapper.CommonMapper.tilVedleggsreferanse;
...
final class SvangerskapspengerMapper {
    ...
    private static LocalDate tilFødselsdato(SvangerskapspengesøknadDto s) {
        if (!(s.barn().fødselsdatoer() == null || s.barn().fødselsdatoer().isEmpty())) {
            return s.barn().fødselsdatoer().get(0);
        }
        return null;
    }
