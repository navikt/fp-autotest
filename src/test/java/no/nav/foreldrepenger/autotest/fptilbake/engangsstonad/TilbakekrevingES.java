...
//        foreslårOgFatterVedtakVenterTilAvsluttetBehandlingOgSjekkerOmBrevErSendt(saksnummer, true);
    ...
    void tilbakeKrevingMedVerge() {
        var familie = FamilieGenerator.ny()
                .forelder(mor(LocalDate.now().minusYears(17))
                        ...
                        .inntektytelse(InntektYtelseGenerator.ny()
                                .ytelse(YtelseType.ARBEIDSAVKLARINGSPENGER, LocalDate.now().minusMonths(1), LocalDate.now().plusMonths(2), 1_000, 100_00)
    ...
    private static LocalDate[] lagFødselsdatoer(EngangsstønadDto engangsstønadDto) {
        var adopsjonDto = (AdopsjonDto) engangsstønadDto.barn();
        return adopsjonDto.fødselsdatoer().toArray(LocalDate[]::new);
    }
}
