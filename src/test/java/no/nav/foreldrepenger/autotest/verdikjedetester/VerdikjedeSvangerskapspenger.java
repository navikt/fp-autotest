...
class VerdikjedeSvangerskapspenger extends VerdikjedeTestBase {
    ...
    void morSøkerIngenTilretteleggingInntektOver6GTest() {
        ...
        var tilrettelegging = TilretteleggingBuilder.ingen(
                LocalDate.now(),
                LocalDate.now(),
                ArbeidsforholdMaler.virksomhet(((Orgnummer) orgnummer)))
                .build();
    ...
    void morSøkerDelvisTilretteleggingMedInntektOver6GTest() {
        ...
        var termindato = LocalDate.now().plusMonths(3);
        var tilrettelegging = TilretteleggingBuilder.delvis(
                LocalDate.now(),
                LocalDate.now(),
                ArbeidsforholdMaler.virksomhet((Orgnummer) orgnummer),
                tilrettelegginsprosent)
                .build();
        var søknad = SøknadSvangerskapspengerMaler.lagSvangerskapspengerSøknad(termindato,
                List.of(tilrettelegging));
        var saksnummer = mor.søk(søknad.build());
    ...
    void morSøkerFørstForATOgSenereForSNTest() {
        ...
        var termindato = LocalDate.now().plusMonths(3);
        var tilrettelegginsprosent = 0.0;
        var tilrettelegging1 = TilretteleggingBuilder.ingen(
                LocalDate.now().minusMonths(2),
                LocalDate.now().minusMonths(2),
                ArbeidsforholdMaler.virksomhet((Orgnummer) arbeidsgiverIdentifikator))
                .build();
        ...
        /* SØKNAD 2 */
        var tilrettelegging2 = TilretteleggingBuilder.ingen(
                LocalDate.now(),
                LocalDate.now(),
                ArbeidsforholdMaler.selvstendigNæringsdrivende())
                .build();
    ...
    void morSøkerIngenTilretteleggingForToArbeidsforholdFullRefusjonTest() {
        ...
        var tilrettelegging2 = TilretteleggingBuilder.ingen(
                LocalDate.now(),
                LocalDate.now(),
                ArbeidsforholdMaler.virksomhet((Orgnummer) orgnummer2))
                .build();
        var søknad = SøknadSvangerskapspengerMaler.lagSvangerskapspengerSøknad(termindato,
    ...
    @Test
    @DisplayName("7: Søknad med avtalt ferieperiode")
    @Description("Søknaden inneholder en avtalt ferieperiode")
    void søknadInneholderAvtaltFerieTest() {
        ...
        var virksomhetDto = ArbeidsforholdMaler.virksomhet(orgnummer);
        var tilrettelegging = TilretteleggingBuilder.ingen(
                        LocalDate.now(),
                        LocalDate.now(),
                        virksomhetDto)
        ...
        var nyTilrettelegging = TilretteleggingBuilder.delvis(
                        LocalDate.now(),
        ...
        var nyAvklarteOppholdsperioder = saksbehandler.hentAksjonspunktbekreftelse(AvklarFaktaFødselOgTilrettelegging.class)
        ...
        assertThat(nyAvklarteOppholdsperioder.getFirst().fom()).isEqualTo(LocalDate.now().plusWeeks(1));
        assertThat(nyAvklarteOppholdsperioder.getFirst().tom()).isEqualTo(LocalDate.now().plusWeeks(2).minusDays(1));
        assertThat(nyAvklarteOppholdsperioder.getLast().fom()).isEqualTo(LocalDate.now().plusWeeks(3));
        assertThat(nyAvklarteOppholdsperioder.getLast().tom()).isEqualTo(LocalDate.now().plusWeeks(4));
    }
