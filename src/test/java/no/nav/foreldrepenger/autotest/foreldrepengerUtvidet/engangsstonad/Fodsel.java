...
class Fodsel extends FpsakTestBase {
    ...
    void morSøkerFødselAvvist() {
        ...
        var mor = familie.mor();
        var søknad = lagEngangstønadFødsel(BrukerRolle.MOR, LocalDate.now().minusDays(30L));
        var saksnummer = mor.søk(søknad.build());
    ...
    void farSøkerFødselRegistrert() {
        ...
        var far = familie.far();
        var søknad = lagEngangstønadFødsel(BrukerRolle.FAR, familie.barn().fødselsdato());
    ...
    // TODO: Hva skjer her?? Fjerner denne hvis det ikke gjøres noe med denne snart.
    @Test
    @Disabled
    ...
    void morSøkerFødselBeregningOverstyrt() {
        ...
        var overstyrBeregning = new OverstyrBeregning(10);
        ...
        var foreslåVedtakBekreftelse = overstyrer
    ...
    void morSøkerFødselFlereBarn() {
        var familie = new Familie("53");
        var mor = familie.mor();
        var fødselsdatoBarn = familie.barn().fødselsdato();
        var søknad = lagEngangstønadFødsel(BrukerRolle.MOR, fødselsdatoBarn)
                .medRelasjonTilBarn(RelasjonTilBarnErketyper.fødsel(2, fødselsdatoBarn));
        var saksnummer = mor.søk(søknad.build());

        saksbehandler.hentFagsak(saksnummer);
        var vurderManglendeFodselBekreftelse = saksbehandler
                .hentAksjonspunktbekreftelse(VurderManglendeFodselBekreftelse.class)
                .bekreftDokumentasjonForeligger(2, LocalDate.now().minusDays(40));
        saksbehandler.bekreftAksjonspunkt(vurderManglendeFodselBekreftelse);
    ...
    void morSøkerFødselMedVerge() {
        ...
        var avklarFaktaVergeBekreftelse = saksbehandler
                ...
                .setVerge(familie.far().fødselsnummer().toString());
        ...
        var avklarBrukerBosattBekreftelse = saksbehandler
                ...
                .setBosattVurderingForAllePerioder(true);
    ...
    void medmorSøkerFødsel() {
        var familie = new Familie("90");
        var medmor = familie.medmor();
        var søknad = lagEngangstønadFødsel(BrukerRolle.MEDMOR, familie.barn().fødselsdato());
